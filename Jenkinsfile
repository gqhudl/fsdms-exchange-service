  ...
  stages {
    ...
    stage('docker build & push image on build docker/build server') {
      ...
      steps {
        ...
        script {
          ...
          if(REMOVE_FLAG){
            // sh 'docker container ls -aq --format {{.ID}} --filter name=fsdms-exchange-ctn | xargs docker container rm -f'
            // sh 'docker container rm -f $(docker container ls -aq --format {{.ID}} --filter name=.*fsdms-exchange-ctn.*)'
        ...
        script {
          ...
          echo "REMOVE_FLAG: ${REMOVE_FLAG}"
          if(REMOVE_FLAG){
            // sh 'docker image ls --format {{.ID}} *${DOCKHUB_USERNAME}/fsdms-exchange-srv* | xargs docker image rm -f'
            // sh 'docker image rm -f $(docker image ls --format {{.ID}} *${DOCKHUB_USERNAME}/fsdms-exchange-srv*)'
        ...
        // script {
          ...
          //   def customImage = docker.build("${DOCKHUB_USERNAME}/fsdms-exchange-srv:latest", '-f Dockerfile .')
    ...
    stage('docker pull image and docker run image on docker/deploy server') {
      ...
      steps {
        ...
        // sh 'docker pull ${DOCKHUB_USERNAME}/fsdms-exchange-srv'

        // docker run images
        sh 'docker run -d -p 9051:9051 -v fsdms-data:/fsdms-data --network fsdms-net --name fsdms-exchange-ctn ${DOCKHUB_USERNAME}/fsdms-exchange-srv -Xms128m -Xmx128m'
      }
    }
